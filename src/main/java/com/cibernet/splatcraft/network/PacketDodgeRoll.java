    ...
    public static class Handler implements IMessageHandler<PacketDodgeRoll, IMessage>
    {
        ...
        void process(PacketDodgeRoll message, MessageContext ctx)
        {
            ...
            if(weapon.getItem() instanceof ItemDualieBase)
            {
                ...
                SplatCraftUtils.createInkExplosion(player.world, player, new BlockPos(player.posX, player.posY, player.posZ), 0.5f, 0, ColorItemUtils.getInkColor(weapon), SplatCraftUtils.getPlayerGlowingInk(player));
            }
        }
    }
}
