...
public class ScoreboardHandler
{
	...
	{
		player.getWorldScoreboard().forAllObjectives(COLOR, player.getScoreboardName(), (p_195397_1_) -> {
			p_195397_1_.setScorePoints(color);
		});
	}
	...
	public static boolean hasColorCriterion(int color) {return COLOR_CRITERIA.containsKey(color);}
	...
	public static Set<Integer> getCriteriaKeySet() {return COLOR_CRITERIA.keySet();}
	
	
	public static CriteriaInkColor getColorKills(int color) {return COLOR_CRITERIA.get(color)[0];}
	public static CriteriaInkColor getDeathsAsColor(int color) {return COLOR_CRITERIA.get(color)[1];}
	public static CriteriaInkColor getKillsAsColor(int color) {return COLOR_CRITERIA.get(color)[2];}
	public static CriteriaInkColor getColorWins(int color) {return COLOR_CRITERIA.get(color)[3];}
	public static CriteriaInkColor getColorLosses(int color) {return COLOR_CRITERIA.get(color)[4];}
	public static CriteriaInkColor[] getAllFromColor(int color) {return COLOR_CRITERIA.get(color);}
	
	public static void register() {}
	
	public static String getColorIdentifier(int color)
	{
		return (InkColor.getByHex(color) == null ? String.format("%06X", color).toLowerCase() : InkColor.getByHex(color).getRegistryName().getPath());
	}
	...
	public static class CriteriaInkColor extends ScoreCriteria
	{
		...
		{
			super((InkColor.getByHex(color) == null ? Splatcraft.MODID : InkColor.getByHex(color).getRegistryName().getNamespace())
					+ "." + name + "." + getColorIdentifier(color));
			this.name = (InkColor.getByHex(color) == null ? Splatcraft.MODID : InkColor.getByHex(color).getRegistryName().getNamespace())
					+ "." + name + "." + getColorIdentifier(color);
			this.color = color;
