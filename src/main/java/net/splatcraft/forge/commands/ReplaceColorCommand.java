...
public class ReplaceColorCommand
{
    public static void register(CommandDispatcher<CommandSource> dispatcher)
    {
        dispatcher.register(Commands.literal("replacecolor").requires(commandSource -> commandSource.hasPermission(2))
                ...
                .then(Commands.argument("stage", new StageIDArgument(true))
                                .then(Commands.argument("color", InkColorArgument.inkColor())
                                        .executes(context -> executeStage(context, 0))
                                        .then(Commands.literal("only").then(Commands.argument("affectedColor", InkColorArgument.inkColor()).executes(context -> executeStage(context, 1))))
                                        .then(Commands.literal("keep").then(Commands.argument("affectedColor", InkColorArgument.inkColor()).executes(context -> executeStage(context, 2))))
                                ))
        );
    }
    ...
    public static int executeStage(CommandContext<CommandSource> context, int mode) throws CommandSyntaxException
    {
        ...
        if (mode == 0)
        {
            return execute(context.getSource(), stage.cornerA, stage.cornerB, InkColorArgument.getInkColor(context, "color"), -1, mode);
        }
        return execute(context.getSource(), stage.cornerA, stage.cornerB, InkColorArgument.getInkColor(context, "color"), InkColorArgument.getInkColor(context, "affectedColor"), mode);
    }
