...
public class BurstBombEntity extends AbstractSubWeaponEntity
{
    ...
    protected void onHitEntity(EntityRayTraceResult result)
    {
        ...
        InkExplosion.createInkExplosion(level, getOwner(), blockPosition(), 2, DAMAGE, DAMAGE, damageMobs, getColor(), inkType, sourceWeapon);
    ...
    {
        InkExplosion.createInkExplosion(level, getOwner(), blockPosition(), EXPLOSION_SIZE, DAMAGE, DAMAGE, damageMobs, getColor(), inkType, sourceWeapon);
        level.broadcastEntityEvent(this, (byte) 1);
        level.playSound(null, getX(), getY(), getZ(), SplatcraftSounds.subDetonate, SoundCategory.PLAYERS, 0.8F, ((level.getRandom().nextFloat() - level.getRandom().nextFloat()) * 0.1F + 1.0F) * 0.95F);
        remove();
    }
